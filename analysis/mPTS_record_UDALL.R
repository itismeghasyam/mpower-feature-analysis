...
demo.tbl <- synapser::synTableQuery('SELECT * FROM syn33706622')$asDataFrame()
tremor.tbl <- synapser::synTableQuery('SELECT * FROM syn33706544')$asDataFrame()
...
# subset to those features used in prediction for mPower
mPower_ftrs <- synapser::synGet('syn50559969')$path %>%
  data.table::fread()

# mpower 2 features
ftrs <- synapser::synGet('syn68243463')$path %>%
  data.table::fread() %>% 
...
ftrs <- ftrs %>% 
  dplyr::filter(feature %in% colnames(mPower_ftrs))
...
# subset for test
# ftrs <- ftrs %>%
# dplyr::filter(recordId %in% mPower_ftrs$recordId)
...
save_to_synapse(
  ...
  output_filename = 'mPTS_per_record_UDALL.tsv',
  parent = 'syn26467775',
  name = 'mPTS UDALL',
