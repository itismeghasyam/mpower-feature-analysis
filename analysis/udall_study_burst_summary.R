############################################################################
# Purpose: Created a study burst schedule for all participants in UDALL UMN Project
#          based on the earliest record in the Health Data Summary Table(syn33706648).
...
HEALTH_DATA_SUMMARY_TABLE <- "syn33706648"
TABLE_OUTPUT_SYNAPSE_ID <- 'syn26467773'
...
fetch_udall <- function() {
  udall <- read_syn_table(HEALTH_DATA_SUMMARY_TABLE)
  udall$createdOnTimeZoneInteger <- unlist(purrr::map(udall$createdOnTimeZone,
...
store_to_synapse <- function(study_burst_summary) {
  OUTPUT_FILE <- 'udall_study_burst_summary.csv' # name your file
  write.csv(study_burst_summary, OUTPUT_FILE)
  synStore(File(OUTPUT_FILE, parentId=TABLE_OUTPUT_SYNAPSE_ID))
  unlink(OUTPUT_FILE)
}
...
main <- function() {
  ...
  study_burst_summary <- build_study_burst_summary(udall) %>% 
