...
burst_info <- synapser::synGet('syn64829296')$path %>% read.csv() %>% dplyr::select(-X)

# get health data summary table
summary.tbl <- synapser::synTableQuery('SELECT * FROM syn33706648')$asDataFrame()
...
train_test_mapping <- list(
  tapping = list(train = "syn25782775", # mpower features
                 test = "syn61529646"), # udall features
  walking = list(train = "syn25782772", # mpower features
                 test = "syn63856123")  # udall features
)

# output reference
OUTPUT_REF <- list(
  filename = "UDALL_PD_conf_score_burst.tsv",
  parent = "syn64829390",
...
save_to_synapse(
  data = pred.tap.rf,
  output_filename = 'tapping_pred_burst.tsv',
...
wal.tbl <- synapser::synTableQuery('SELECT * FROM syn33706639')$asDataFrame()
...
save_to_synapse(
  data = pred.wal.rf,
  output_filename = 'walking_pred_burst.tsv',
  parent = OUTPUT_REF$parent,
  # annotations = OUTPUT_REF$annotations,
  # name = OUTPUT_REF$name,
  # description = OUTPUT_REF$description,
...
###########################################
mPTS <- synapser::synGet('syn68243637')$path %>% 
...
demo <- synapser::synTableQuery('SELECT * FROM syn33706622')$asDataFrame() %>% 
  dplyr::select(externalId, healthCode, sex, diagnosis, phoneInfo, birthYear)

mPTS <- mPTS %>% 
  dplyr::filter(healthCode %in% demo$healthCode)
